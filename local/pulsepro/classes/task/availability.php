...
class availability extends \core\task\adhoc_task {
    ...
    public function update_availability() {
        ...
        if (!empty($instance->students)) {
            ...
            $limit = get_config('local_pulsepro', 'tasklimituser') ?: 500;
            if (count($students) > $limit) {
                return true;
            }
            $availabilityrecords = $this->fetch_availability_records($pulseid, $students);
            if (isset($instance->cm_array)) {
                pulse_mtrace('Update availability init for the instance - '. $instance->cm_array->id);
                ...
                if (!empty($insertrecords)) {
                    $DB->insert_records('local_pulsepro_availability', $insertrecords);
                }
            }
        }
        return true;
    ...
    public function fetch_availability_records($pulseid, $students) {
        ...
        $sql = 'SELECT * FROM {local_pulsepro_availability} WHERE pulseid=:pulseid and userid '.$insql;
    ...
    public static function fetch_available_users($pulseid) {
        ...
        $records = $DB->get_records('local_pulsepro_availability', $params);
        $newrecord = [];
        foreach ($records as $key => $record) {
            $newrecord[$record->userid] = $record;
        }
